...
def render_page(active: str = "changelog") -> str:
    import shell
    releases = []
    for rel in CHANGELOG:
        items = []
        for c in rel["changes"]:
            label, cls = _PILL.get(c["type"], (c["type"].title(), "feat"))
            items.append(
                '<li class="citem">'
                f'<span class="cpill {cls}">{_h.escape(label)}</span>'
                '<div class="ctext">'
                f'<div class="ctitle">{_h.escape(c["title"])}</div>'
                f'<p class="cdetail">{_h.escape(c["detail"])}</p>'
                '</div></li>')
        releases.append(
            '<section class="release">'
            f'<h2 class="rdate">{_h.escape(rel["date"])}'
            f'<span class="rcount">{len(rel["changes"])} changes</span></h2>'
            f'<ul class="clist">{"".join(items)}</ul>'
            '</section>')

    return f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>What's new — Constructor Insight</title>
<style>
  :root{{--bg:#f6f8fa;--panel:#fff;--panel2:#eaeef2;--line:#d0d7de;--ink:#1f2328;
    --mut:#656d76;--acc:#0969da;--good:#1a7f37;--app:#8250df;--warn:#9a6700}}
  *{{box-sizing:border-box}}
  body{{margin:0;background:var(--bg);color:var(--ink);
    font:15px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}}
  {shell.SHELL_CSS}
  .wrap{{padding:20px 24px 80px;max-width:820px}}
  h1{{font-size:24px;margin:0 0 4px}} .sub{{color:var(--mut);font-size:13px;margin:0 0 22px}}
  .release{{margin:0 0 30px}}
  .rdate{{font-size:15px;margin:0 0 14px;padding-bottom:8px;border-bottom:1px solid var(--line);
    display:flex;align-items:baseline;gap:10px}}
  .rcount{{color:var(--mut);font-weight:600;font-size:12px}}
  ...
  .cpill{{flex:0 0 auto;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.03em;
    padding:3px 9px;border-radius:999px;margin-top:2px;min-width:64px;text-align:center}}
  .cpill.sec{{background:rgba(154,103,0,.15);color:var(--warn)}}
  .cpill.fix{{background:rgba(26,127,55,.14);color:var(--good)}}
  .cpill.feat{{background:rgba(9,105,218,.12);color:var(--acc)}}
  .cpill.des{{background:rgba(130,80,223,.14);color:var(--app)}}
  .ctext{{min-width:0}}
  .ctitle{{font-weight:700;font-size:14px}}
  .cdetail{{color:var(--mut);font-size:13px;margin:3px 0 0}}
  {shell.BASE_CSS}
</style></head>
<body><div class="app">{shell.sidebar_html(active)}<main class="wrap">
<h1>What's new</h1>
<p class="sub">Notable changes to the report and portal, newest first.</p>
{"".join(releases)}
</main></div></body></html>"""
